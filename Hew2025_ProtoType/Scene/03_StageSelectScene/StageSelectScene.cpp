...
{    
    ...
    this->p_obj->SetSize(100.0f, 100.0f, 0.0f);
...
void	StageSelectScene::Update(void)
{
    ...
    // 当たっていたら色変る
    auto& coll = this->p_obj->GetCollider();

    if (this->p_point->CheckCollision(coll))
    {
        std::cout << "当たった" << std::endl;
        this->p_obj->SetColor(DirectX::XMFLOAT4(1.0f, 1.0f, 0.0f, 1.0f));
    }
    else {
        this->p_obj->SetColor(DirectX::XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f));
    }

    if (this->p_input->Press("SPACE"))
    {
        this->p_point->SetPosition(DirectX::XMFLOAT3(200.0f, 200.0f, 0.0f));
    }
    else
    {
