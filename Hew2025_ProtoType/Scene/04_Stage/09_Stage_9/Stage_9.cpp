...
void	Stage_9::Initialize(void)
{
    ...
    if (!this->pen[0]) { this->pen[0] = new Pen(this->p_camera); }
    this->pen[0]->Init(L"Asset/Gimmick/pen.png");
    this->pen[0]->SetPos(this->PenPos[0].x, this->PenPos[0].y, 0.0f);
    this->pen[0]->SetSize(this->PenSize[0].x, this->PenSize[0].y, 0.0f);
    this->pen[0]->SetAngle(180.0f);

    if (!this->pen[1]) { this->pen[1] = new Pen(this->p_camera); }
    this->pen[1]->Init(L"Asset/Gimmick/pen.png");
    this->pen[1]->SetPos(this->PenPos[1].x, this->PenPos[1].y, 0.0f);
    this->pen[1]->SetSize(this->PenSize[1].x, this->PenSize[1].y, 0.0f);
    ...
    player->SetPos(300.0f, -4650.0f, 0.0f);
    //player->SetPos(4650.0f, -1450.0f, 0.0f);
   // player->SetPos(700, -3570.0f, 0.0f);
    Vx = 0.0f;
    Vy = 0.0f;
    t = 0;
    t2 = 0;
...
void	Stage_9::Update(void)
{
    ...
    this->PushObject->SetColliderSize(DirectX::XMFLOAT3(350.0f, 850.0f, 0.0f));
    ...
    this->hook[1]->SetColliderSize(DirectX::XMFLOAT3(HookColSize01.x + 5.0f, HookColSize01.y + 11.0f, 0.0f));
    ...
    auto& pushColl1 = PushObject->GetCollider();
    ...
    if (col4.CheckCollision(SlopeColl4))
    {
        PlayerColState2 = 1;//block collider
    ...
    if (playerColl.CheckCollision(SlopeColl2))
    {
        ColliderState = 1;
        SlopeState2 = 1;

    }
    if (col2.CheckCollision(SlopeColl2))
    {
        PlayerColState = 1;
        // SlopeState2 = 1;
    }
    if (playerColl.CheckCollision(SlopeColl4))
    {
        ColliderState = 1;
        SlopeState4 = 1;

    }
    if (col2.CheckCollision(SlopeColl4))
    {
        PlayerColState = 1;
        // SlopeState2 = 1;
    }
    ...
    if (gamemode == 1)
    {
        ...
        // ========================================
        // 降下したら死ぬ！！
        if (playerPos.y <= -5500.0f && (!this->player->GetIsDead()))
        {
            // ターゲットの解除
        ...
        else if (PenColState == 1 && PenPos[0].y - 70 - 200 * sin(PenAngle0 * 3.1415926f / 180.0f) > -4364.0f && PenAngle0 > -180.0f && PenAngle0 < 60.0f)
        {
        ...
        else if (PenColState2 == 1 && PenPos[1].y - 70 - 200 * sin(PenAngle1 * 3.1415926f / 180.0f) > -4364.0f && PenAngle1 > -180.0f && PenAngle1 < 60.0f)
        {
        ...
        if (this->p_input->Press("LEFT"))
        {
            ...
            if (movestate != 1)
            {
                if (SlopeState1 == 1)
                {
                    this->player->SetPos(playerPos.x - 3.5f, playerPos.y - 3.5f, playerPos.z);
                }
                else
                {
        ...
        if (grabstate == 0)
        {
            ...
            else
            {
                ...
                else if (p_input->GetLeftAnalogStick().x * 10.0f < -2.0f && superjumpstate == 0)
                {
                    ...
                    if (SlopeState1 == 1)
                    {
                        this->player->SetPos(playerPos.x + Vx4 * 0.7, playerPos.y - Vx4 * 0.7, playerPos.z);
                    }
                    else  if (SlopeState2 == 1)
                    {
                        this->player->SetPos(playerPos.x + Vx4 * 0.6, playerPos.y + Vx4 * 1.0392, playerPos.z);
                    }
                    else  if (SlopeState3 == 1)
                    {
                        this->player->SetPos(playerPos.x + Vx4 * 0.7, playerPos.y - Vx4 * 0.7, playerPos.z);
                    }
                    else  if (SlopeState4 == 1)
                    {
                        if (StopState != 1)
                        {
                            this->player->SetPos(playerPos.x + Vx4 * 0.65, playerPos.y + Vx4 * 0.9282, playerPos.z);
                        }
                    }
                    ...
                    if (movestate != 1)
                    {
                        if (SlopeState1 == 1)
                        {
                            // this->player->SetPos(playerPos.x + Vx4/2, playerPos.y + Vx4 / 2, playerPos.z);
                        }
                        else if (SlopeState2 == 1)
                        {
        ...
        else if (TestPenState == 1 || TestPen2State == 1)
        {
            superkeystate = 0;
        }
        else
        ...
        if (this->p_input->Press("SUPERJUMP") && t == 0 && grabstate == 1 && deathstate != 1 && superpressstate != 1)
        {
            superpressstate = 1;
            superkeystate = 1;
