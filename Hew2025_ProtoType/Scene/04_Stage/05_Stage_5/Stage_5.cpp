...
void	Stage_5::Initialize(void)
{
    ...
    for (n = 0; n < 2; n++)
    {
        if (!this->YoyoObject[n]) { this->YoyoObject[n] = std::make_shared<Object>(this->p_camera); }
        this->YoyoObject[n]->Init(L"Asset/Gimmick/yo-yo.png");
    }
    for (n = 0; n < 3; n++)
    {
        if (!this->rail[n]) { this->rail[n] = std::make_shared<Object>(this->p_camera); }
    ...
    if (!this->pendulum) { this->pendulum = std::make_shared<Pendulum>(this->p_camera); }
    if (!this->grabbox) { this->grabbox = std::make_shared<GrabBox>(this->p_camera); }
    ...
    for (n = 0; n < 7; n++)
    {
        ...
        this->hookdraw[n]->SetSize(HookSize02.x, HookSize02.y, 0.0f);
    }

    this->rail[1]->SetAngle(90);
...
void	Stage_5::Update(void)
{
    ...
    if (gamemode == 0)
    {
        ...
        {
            this->player->SetPos(playerPos.x - 20.0f, playerPos.y, playerPos.z);
        }
        ...
        {
            this->player->SetPos(playerPos.x, playerPos.y + 20.0f, playerPos.z);
        }
   ...
   //フックの移動処理
    ...
    if (playerColl.CheckCollision(colhook[0]) || playerColl.CheckCollision(colhook[1]) || playerColl.CheckCollision(colhook[2]) || playerColl.CheckCollision(colhook[3]) || playerColl.CheckCollision(colhook[4]) || playerColl.CheckCollision(colhook[5]) || playerColl.CheckCollision(colhook[6]))
    {
        //std::cout << "当たった" << std::endl;
        //ColliderState = 3;
        ...
        if (playerColl.CheckCollision(colhook[3]))
        {
            MoveHookFLG[1] = true;
        }
        if (playerColl.CheckCollision(colhook[5]))
        {
            MoveHookFLG[2] = true;
        }
    }
    ...
    if (gamemode == 1)
    {
        ...
        // ========================================
        // 降下したら死ぬ！！
        if (playerPos.y <= -6100.0f && (!this->player->GetIsDead()))
        {
            // ターゲットの解除
        ...
        this->hookdraw[6]->SetPos(HookPos06.x, HookPos06.y + 80.0f, playerPos.z);
        ...
        if (ScenechangeState2 == 1)
        {
            ...
            this->p_sceneManager->ChangeScene(Scene::Stage_8);// Stage8に遷移!!!!!!
            return;
        }
