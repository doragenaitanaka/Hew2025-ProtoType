...
void	Stage_10::Initialize(void)
{
    ...
    if (!this->p_tileMap)
    {
        ...
        this->p_tileMap->GenerateMap("Stage10.csv");
    }
    ...
    this->rail[3]->SetPos(RailPos03.x, RailPos03.y, 0.0f);
    this->rail[4]->SetPos(RailPos04.x, RailPos04.y, 0.0f);
    this->rail[5]->SetPos(RailPos05.x, RailPos05.y, 0.0f);
    player->SetPos(7400.0f, -1650.0f, 0.0f);
    Vx = 0.0f;
    Vy = 0.0f;
    t = 0;
    t2 = 0;
...
void	Stage_10::Update(void)
{
   ...
   //左から順
    ...
    this->hook[4]->SetColliderSize(DirectX::XMFLOAT3(HookSize01.x, HookSize01.y, 0.0f));
    this->hook[5]->SetColliderSize(DirectX::XMFLOAT3(HookSize01.x, HookSize01.y, 0.0f));
   ...
   hook[5]->GetCollider()
    ...
    if (playerColl.CheckCollision(colhook[0]) || playerColl.CheckCollision(colhook[1]) || playerColl.CheckCollision(colhook[2]) || playerColl.CheckCollision(colhook[3]) || playerColl.CheckCollision(colhook[4]) || playerColl.CheckCollision(colhook[5]))
    {
        //std::cout << "当たった" << std::endl;
      //  ColliderState = 3;
    ...
    if (gamemode == 1)
    {
        ...
        this->goal->SetPos(350.0f, -1750.0f, playerPos.z);
